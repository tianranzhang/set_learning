...
def training(train_data, val_data, test_data, onehot_train, onehot_val, onehot_test, embedding_matrix, curr_cross_val = 0, 
  ...
  # fix random seed for reproducibility
    ...
    #o2 = GRU(cnn_dim,dropout = 0.2)(embedded_sequences)
...
def main_pipeline (perm = 'noperm', perm_file = 'None', lr = 0.001, epoch_num = 10, cnn_dim = 256, ksize = 2, res_block = 1, len_seq = 256, skip_gram = 1, dim = 50, win_size = 20, run_num = 0):
	...
	for curr_cross_val in np.arange(1):
		predicted_val, predicted_test,scores_val,scores_test = training(train_x, val_x, test_x, train_y, val_y, test_y, embedding_matrix,
			dim = dim, len_seq = len_seq, cnn_dim = cnn_dim, ksize=ksize,res_block = res_block, epoch_num = epoch_num, lr = lr)
	...
	with open("tcn_abnormlabs_baseline/"+"exp_logs_tcn.csv", 'a', newline='') as csvFile: 
		writer = csv.DictWriter(csvFile, fieldnames=['acc_val','auc_vals','prec_val','rec_val', 'acc_test','auc_test','prec_test','rec_test',"prauc_vals","prauc_test","dim",
    		"cnn_dim","len_seq", "perm","lr","epoch_num","len_seq", "curr_dim","ksize", "res_block","win_size", "perm_file","run_num"])
		writer.writerow({'acc_val': str(np.mean(acc_val)),'auc_vals': str(np.mean(auc_vals)),'prec_val': str(np.mean(prec_val)),'rec_val': str(np.mean(rec_val)),
                    	...
                    	"len_seq": str(len_seq), "curr_dim":str(dim), "ksize": ksize,"res_block": res_block, "win_size": str(win_size), "perm_file": str(perm_file), "run_num": str(run_num)})
	csvFile.close()
				...
				for win_size in [20, 10, 5]:
					for perm in ['tcn_abnormlabs_baseline/permutation_1_10_label', 'noperm', 'tcn_abnormlabs_baseline/permutation_1_1_label', 'tcn_abnormlabs_baseline/permutation_1_2_label']:
						...
						main_pipeline (perm = perm, perm_file = 'None', lr = lr, epoch_num = epoch_num, cnn_dim = cnn_dim, 
							ksize = ksize, res_block = res_block, len_seq = len_seq, skip_gram = skip_gram, dim = dim, win_size = win_size, run_num = run_num)
