...
def training(train_data,  val_data, test_data, onehot_train, onehot_val, onehot_test, embedding_matrix, curr_cross_val = 0, 
    ...
    m.fit(np.array(train_data), np.array(onehot_train), validation_data = (np.array(val_data),  np.array(onehot_val)), epochs = epoch_num, batch_size = 64, verbose = 2, callbacks=callbacks_list) #callbacks=callbacks_list #validation_data=([np.array(val_data_1), np.array(val_data_2)],  [np.array(onehot_val_1), np.array(onehot_val_1)-np.array(onehot_val_1)]), 
...
def main_pipeline(perm = 'noperm', perm_file = 'None', lr = 0.001, epoch_num = 10, cnn_dim = 256,  len_seq = 256, skip_gram = 1, dim = 50, win_size = 20, run_num = 0):
	...
	with open("tcn_abnormlabs_baseline/"+"exp_logs_gru_original_tested_on_original_fixed_w2v.csv", 'a', newline='') as csvFile:
		...
		writer.writerow({'acc_val': str(np.mean(acc_val)),'auc_vals': str(np.mean(auc_vals)),'prec_val': str(np.mean(prec_val)),'rec_val': str(np.mean(rec_val)),
                    	...
                    	"len_seq": str(len_seq), "curr_dim":str(dim),  "win_size": str(win_size), "perm_file": str(perm_file),"note":'original_with_vanilla_gru', "run_num": str(run_num)})
	csvFile.close()
...
for len_seq in [512, 128,256,512]:#128,256,
	for perm in ['noperm']:#'perm', 
		for cnn_dim in [64,128,256]:#,64,256
