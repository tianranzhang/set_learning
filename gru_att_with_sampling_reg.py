...
#ef training(train_data,  val_data, test_data, onehot_train, onehot_val, onehot_test, embedding_matrix, curr_cross_val = 0, 
             #dim = 256, len_seq = 50, cnn_dim = 200, ksize=2,res_block = 1, epoch_num = 20, lr = 0.001):
def training(train_data_1, train_data_2, val_data_1,val_data_2, test_data_1, test_data_2, onehot_train_1, onehot_train_2, onehot_val_1, onehot_val_2, onehot_test_1, onehot_test_2, embedding_matrix, curr_cross_val = 0, 
             dim = 256, len_seq = 50, cnn_dim = 200, ksize=2,res_block = 1, epoch_num = 20, lr = 0.001):
    ...
    sequence_input_1 = Input(shape = (len_seq, ), dtype = 'int32')
    ...
    embedded_sequences_1 = emb_layer(sequence_input_1)
    embedded_sequences_2 = emb_layer(sequence_input_2)
    

    gru_layer = GRU(cnn_dim,dropout = 0.2,return_sequences=True)
    #o = gru_layer(embedded_sequences)
    o_1 = gru_layer(embedded_sequences_1)
    ...
    sub = K.abs(tensorflow.keras.layers.Subtract()([x_1,x_2]))
    sub = Reshape((cnn_dim, 1))(sub)
    mae = cnn_dim*GlobalAveragePooling1D()(sub)
    mae = Reshape([1])(mae)
    ...
    m = Model(inputs = [sequence_input_1,sequence_input_2], outputs = [o,mae])
    ...
    m.compile(optimizer=optimizers.Adam(lr=lr),loss = ['binary_crossentropy', 'mae'], 
    metrics= ['accuracy'], loss_weights=[1.00,0.01])
    print(m.summary())
    ...
    callbacks_list = [tensorflow.keras.callbacks.EarlyStopping(monitor='val_loss', patience=2, restore_best_weights=True)]
    ...
    #sampling for no reg:
        ...
        #random_indices = np.random.choice(number_of_rows, size=5426*20, replace=False)
        #train_data_sample = np.array(train_data)[random_indices, :]
        #onehot_train_sample = np.array(onehot_train)[random_indices]
    #sampling for reg:
        number_of_rows = np.array(train_data_1).shape[0]
        random_indices = np.random.choice(number_of_rows, size=5426*20, replace=False)
        train_data_1_sample = np.array(train_data_1)[random_indices, :]
        train_data_2_sample = np.array(train_data_2)[random_indices, :]
        onehot_train_1_sample = np.array(onehot_train_1)[random_indices]



        #number_of_rows = np.array(val_data).shape[0]
        #random_indices = np.random.choice(number_of_rows, size=904*20, replace=False)
        #val_data_sample = np.array(val_data)[random_indices, :]
        #onehot_val_sample = np.array(onehot_val)[random_indices]


        #sampling for reg:
        number_of_rows = np.array(val_data_1).shape[0]
        random_indices = np.random.choice(number_of_rows, size=904*20, replace=False)
        val_data_1_sample = np.array(val_data_1)[random_indices, :]
        val_data_2_sample = np.array(val_data_2)[random_indices, :]
        onehot_val_1_sample = np.array(onehot_val_1)[random_indices]
        ...
        m.fit([np.array(train_data_1_sample), np.array(train_data_2_sample)], [np.array(onehot_train_1_sample), np.zeros([len(onehot_train_1_sample),1])], 
        validation_data=([np.array(val_data_1_sample), np.array(val_data_2_sample)], [np.array(onehot_val_1_sample), np.zeros([len(onehot_val_1_sample),1])]),
        epochs= 1, batch_size=128, verbose = 2, callbacks= callbacks_list)
        ...
        predicted_val_updated, _ = m.predict([np.array(val_data_1), np.array(val_data_2)])
        fpr, tpr, _ = roc_curve(onehot_val_1,predicted_val_updated)
        val_auc = auc(fpr, tpr)
        ...
        if val_auc>best_auc:
            best_auc = val_auc
            best_model = m
            predicted_val, _ = best_model.predict([np.array(val_data_1), np.array(val_data_2)])
            predicted_test, _ = best_model.predict([np.array(test_data_1), np.array(test_data_2)])
...
def main_pipeline(perm = 'noperm', perm_file = 'None', lr = 0.001, epoch_num = 10, cnn_dim = 256,  len_seq = 256, skip_gram = 1, dim = 50, win_size = 20, run_num = 0):
	...
	if perm == 'noperm':
		...
		#matched_data = pd.read_csv("tcn_abnormlabs_baseline/matched_100_original.csv",sep = ',')
		...
		#matched_train_x.to_csv(perm_file+'_matched_original_train_x_'+str(len_seq)+'.csv',  index=False)
		...
		#matched_val_x.to_csv(perm_file+'_matched_original_val_x_'+str(len_seq)+'.csv',  index=False)
		...
		#matched_test_x.to_csv(perm_file+'_matched_original_test_x_'+str(len_seq)+'.csv',  index=False)
	...
	if perm_file == 'None':
		...
		#pd.DataFrame(np.array(matched_train_x)).to_csv(perm_file+'_matched_original_train_x_encoded'+str(len_seq)+'.csv', index=False)
		...
		#pd.DataFrame(np.array(matched_val_x)).to_csv(perm_file+'_matched_original_val_x_encoded'+str(len_seq)+'.csv', index=False)
		...
		#pd.DataFrame(np.array(matched_test_x)).to_csv(perm_file+'_matched_original_test_x_encoded'+str(len_seq)+'.csv', index=False)
		#print("perm val data encoded saved")
	...
	for curr_cross_val in np.arange(1):
		...
		predicted_val, predicted_test = training(train_x, matched_train_x, val_x, matched_val_x, test_x, matched_test_x, train_y, train_y, val_y, val_y, test_y, test_y, embedding_matrix,
	...
	with open("tcn_abnormlabs_baseline/"+"exp_logs_gru_perm_with_sampling_reg_cnn_512_tested_on_original_fixed_w2v.csv", 'a', newline='') as csvFile:
		...
		writer.writerow({'acc_val': str(np.mean(acc_val)),'auc_vals': str(np.mean(auc_vals)),'prec_val': str(np.mean(prec_val)),'rec_val': str(np.mean(rec_val)),
						...
						"len_seq": str(len_seq), "curr_dim":str(dim),  "win_size": str(win_size), "perm_file": str(perm_file),"note":'reg1_0.01+perm_with_sampling', "run_num": str(run_num)})
	csvFile.close()
...
for len_seq in [512,256]:#128,256,
	for perm in ['perm']:#'perm', 
		for cnn_dim in [512]:#,64,256
