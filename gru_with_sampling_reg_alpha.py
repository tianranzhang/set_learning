...
def main_pipeline(perm = 'noperm', perm_file = 'None', lr = 0.001, epoch_num = 10, cnn_dim = 256,  len_seq = 256, skip_gram = 1, dim = 50, win_size = 20, run_num = 0):
	...
	#2) when permutation
		...
		alpha_data_pre = pd.read_csv('tcn_abnormlabs_baseline/permutation_percent_1_1_unique0_orderingalpha_label.csv', sep = ',')
		print(alpha_data_pre.head())
		test_data = alpha_data_pre[alpha_data_pre.subject_id.isin(test_list.subject_id)]
		del alpha_data_pre
	...
	if perm == 'noperm':
		...
		#matched_train_x = pd.read_csv(perm_file+'_matched_alpha_train_x_'+str(len_seq)+'.csv', sep = '\t')['seq'].tolist()
		...
		#matched_val_x = pd.read_csv(perm_file+'_matched_alpha_val_x_'+str(len_seq)+'.csv', sep = '\t')['seq'].tolist()
		...
		#matched_test_x = pd.read_csv(perm_file+'_matched_alpha_test_x_'+str(len_seq)+'.csv', sep = '\t')['seq'].tolist()
	...
	with open("tcn_abnormlabs_baseline/"+"exp_logs_gru_perm_with_sampling_reg_cnn_512_tested_on_alpha_fixed_w2v.csv", 'a', newline='') as csvFile:
		...
		writer.writerow({'acc_val': str(np.mean(acc_val)),'auc_vals': str(np.mean(auc_vals)),'prec_val': str(np.mean(prec_val)),'rec_val': str(np.mean(rec_val)),
						...
						"len_seq": str(len_seq), "curr_dim":str(dim),  "win_size": str(win_size), "perm_file": str(perm_file),"note":'reg1_0.01+perm_with_sampling_alpha', "run_num": str(run_num)})
	csvFile.close()
