...
def training(train_data,  val_data, test_data, onehot_train, onehot_val, onehot_test, embedding_matrix, curr_cross_val = 0, 
    ...
    m.fit(train_data, onehot_train, validation_data = (val_data,  onehot_val), epochs = 100, batch_size = 128, verbose = 2, callbacks=callbacks_list)  #validation_data=([np.array(val_data_1), np.array(val_data_2)],  [np.array(onehot_val_1), np.array(onehot_val_1)-np.array(onehot_val_1)]), 
...
def main_pipeline(perm = 'noperm', perm_file = 'None', lr = 0.001, epoch_num = 10, cnn_dim = 256,  len_seq = 256, skip_gram = 1, dim = 50, win_size = 20, run_num = 0):
	...
	with open("tcn_abnormlabs_baseline/"+"exp_logs_gru_atttest_on_alpha.csv", 'a', newline='') as csvFile:
		...
		writer.writerow({'acc_val': str(np.mean(acc_val)),'auc_vals': str(np.mean(auc_vals)),'prec_val': str(np.mean(prec_val)),'rec_val': str(np.mean(rec_val)),
                    	...
                    	"len_seq": str(len_seq), "curr_dim":str(dim),  "win_size": str(win_size), "perm_file": str(perm_file), 'note': 'gru_att_512_on_alpha',"run_num": str(run_num)})
	csvFile.close()
