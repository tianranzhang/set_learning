...
def training(train_data_1, train_data_2, val_data_1,val_data_2, test_data_1, test_data_2, onehot_train_1, onehot_train_2, onehot_val_1, onehot_val_2, onehot_test_1, onehot_test_2, embedding_matrix, curr_cross_val = 0, 
    ...
    metrics= ['accuracy'], loss_weights=[1.00,0.01])#,tensorflow.keras.metrics.AUC(), tensorflow.keras.metrics.Precision(), tensorflow.keras.metrics.Recall()]) #
    print(m.summary())
    ...
    m.fit([np.array(train_data_1), np.array(train_data_2)], [np.array(onehot_train_1), np.zeros([len(onehot_train_1),1])], 
    	validation_data=([np.array(val_data_1), np.array(val_data_2)],  [np.array(onehot_val_1), np.zeros([len(onehot_val_1),1])]),
    	epochs=epoch_num, batch_size=128, verbose = 2, callbacks= callbacks_list)  #validation_data=([np.array(val_data_1), np.array(val_data_2)],  [np.array(onehot_val_1), np.array(onehot_val_1)-np.array(onehot_val_1)]), 
    ...
    predicted_val, _ = m.predict([np.array(val_data_1), np.array(val_data_2)])
    predicted_test, _ = m.predict([np.array(test_data_1), np.array(test_data_2)])
...
def main_pipeline(perm = 'noperm', perm_file = 'None', lr = 0.001, epoch_num = 10, cnn_dim = 256,  len_seq = 256, skip_gram = 1, dim = 50, win_size = 20, run_num = 0):
	...
	with open("tcn_abnormlabs_baseline/"+"exp_logs_gru_att_regularizationl1_p1_100_unique0_fixed_w2v.csv", 'a', newline='') as csvFile:
		...
		writer.writerow({'acc_val': str(np.mean(acc_val)),'auc_vals': str(np.mean(auc_vals)),'prec_val': str(np.mean(prec_val)),'rec_val': str(np.mean(rec_val)),
                    	...
                    	"len_seq": str(len_seq), "curr_dim":str(dim),  "win_size": str(win_size), "perm_file": str(perm_file), 'note': 'batch_size=128',"run_num": str(run_num)})
	csvFile.close()
